...
功能：
- 天气查询（高德天气 Amap API，需配置 AMAP_API_KEY）
...
_spec = importlib.util.spec_from_file_location(
    "query_info_agent",
    os.path.join(project_root, 'skills', 'query-info', 'script', 'agent.py'),
)
_mod = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_mod)
InformationQueryAgent = _mod.InformationQueryAgent


pytestmark = pytest.mark.integration  # 真实调用 LLM/网络，无凭证时由 conftest 跳过
...
async def test_information_query_agent():
    ...
    print()
    print("使用完全免费的API：")
    print("• 高德天气 Amap（需配置 AMAP_API_KEY）")
    ...
    print()
    print("提示：")
    print("• 天气查询: 使用高德天气 Amap API（需配置 AMAP_API_KEY）")
