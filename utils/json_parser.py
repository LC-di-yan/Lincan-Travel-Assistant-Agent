...
def robust_json_parse(text: str, fallback=None) -> dict:
    ...
    try:
        ...
        logger.warning(f"Direct JSON parse failed: {last_error}")
        start = max(0, error_pos - 50)
        end = min(len(json_str), error_pos + 50)
        logger.warning(f"Error context: ...{json_str[start:end]}...")

    # 尝试1.5: 修复中文引号（LLM 常见输出问题）
    try:
        json_str_cn = json_str.replace('“', '"').replace('”', '"')  # ""
        json_str_cn = json_str_cn.replace('‘', "'").replace('’', "'")  # ''
        result = json.loads(json_str_cn)
        logger.info("JSON parsed successfully after fixing Chinese quotes")
        return result
    ...
    try:
        ...
        last_error = str(e)
        logger.warning(f"JSON parse failed after cleaning: {last_error}")
    ...
    try:
        ...
        last_error = str(e)
        logger.warning(f"JSON parse failed after fixing quotes: {last_error}")
    ...
    try:
        ...
        last_error = str(e)
        logger.warning(f"JSON parse failed after removing trailing commas: {last_error}")
    ...
    try:
        ...
        last_error = str(e)
        logger.warning(f"JSON parse failed after smart escaping: {last_error}")
    ...
    try:
        ...
        last_error = str(e)
        logger.warning(f"JSON5 parse failed: {last_error}")
